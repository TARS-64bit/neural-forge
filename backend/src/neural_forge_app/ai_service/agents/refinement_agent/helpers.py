...
def splice_and_remap_tasks(
    ...
    max_existing_id = max([t.task_id for t in plan_tasks], default=0)
    ...
    for sub_task in new_sub_tasks:
        max_existing_id += 1
        id_mapping[sub_task.task_id] = max_existing_id
        sub_task.task_id = max_existing_id
    ...
    updated_plan = [t for t in plan_tasks if t.task_id != vague_id]
    updated_plan.extend(new_sub_tasks)

    # Optional: Sort by task_id for readability in debugging
    updated_plan.sort(key=lambda t: t.task_id)
